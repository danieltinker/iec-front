<template>
    <div :style="'background-color:' + getCurrentTheme.cyber_status.color_1" style="position: relative">
        ...
        <div style="height:400px; " v-if="doneFetching">
            ...
            <v-row id="test" class="mt-2" :style="{ color: getCurrentTheme.cyber_status.box_color_1 }"
                ...
                <div class="eachBar" v-for="(bar, key) in prodData" :key="key">
                    <div id="100red" style="height:1px">
                        <span v-if="key == 0" :style="{ color: getCurrentTheme.cyber_status.color_1 }"
                            style="display: block; z-index: 1; position: relative; top: 206px; ">XXX</span>
                        <span v-if="key == 1"
                    ...
                    <div id="600orange" style="height:1px">
                        <span v-if="key == 0" :style="{ color: getCurrentTheme.cyber_status.color_1 }"
                            style="display: block; z-index: 1; position: relative; top: 172px; ">XXX</span>
                        <span v-if="key == 1"
                    ...
                    <div id="1100yellow">
                        <span v-if="key == 0" :style="{ color: getCurrentTheme.cyber_status.color_1 }"
                            style="display: block; z-index: 1; position: relative; top: 129px; ">XXX</span>
                        <span v-if="key == 1"
                    ...
                    <v-col style="display: block; z-index: 2; position: relative;">
                        <div style="position: relative;  text-align: center; ">
                            <label style="font-size:17px; font-family: almoni;">{{ bar.value.toLocaleString("en-US")
                            }}</label>
                            <OneBar :data="bar" :params="params" backgroundColor="rgba(0, 0, 100%, 80%)"
                                cWidth="55px" />
                            <div style="display:flex; flex-direction:column;">
                                <label style="font-size:20px; font-family: almoni; bottom:0; ">{{ bar.label }}</label>
                                <label style="font-size:20px; font-family: almoni; bottom:0; ">({{
                                        updateLabel(bar.description)
                                }})</label>
                            </div>
                        </div>
                    </v-col>
