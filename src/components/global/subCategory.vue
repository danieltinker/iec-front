  ...
  export default {
    ...
    methods: {
      setCategory(index, category) {
        console.log("dssssssssssssssssssssss",this.$store.state.selected_sub_cat_id);
        this.active = index;
        this.$store.state.selected_sub_cat_id = category;
      }
    },
    async created(){
      await axios
            .get(this.$store.state.serverAdrr+"/shavit-mobile"+
              `/sub_categories/${this.$store.state.selected_hq_id}`,
